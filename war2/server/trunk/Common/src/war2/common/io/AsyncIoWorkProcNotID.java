 ...
 * 
 */
class AsyncIoWorkProcNotID implements IIoWorkProc<IIoWorkNotID> {
	/** 处理器名称 */
	private static final String PROCESSOR_NAME = "xgame::AsyncIoWorkProcNotID";
	/** 最大线程数量 */
	 ...
	 * 
	 */
	public AsyncIoWorkProcNotID(IoWorkService workServ) {
		if (workServ == null) {
	...
	@Override
	public void startWork(IIoWorkNotID work) {
		if (work == null) {
			return;
		}
	...
	private void callAsyncProc(StatefulIoWork work) {
		...
		// 获取无 ID 的工作
		IIoWorkNotID workNotID = (IIoWorkNotID)work.getInnerWork();
		...
		// 获取运行器
		IoWorkRunner runner = this.getWorkRunner(this._runnerArray);
	 ...
	 * 
	 */
	private IoWorkRunner getWorkRunner(IoWorkRunner[] runnerArray) {
		if (runnerArray == null || 
			runnerArray.length <= 0) {
			return null;
		}

		int index = (int)(Math.random() * runnerArray.length);
	...
	private static class IoWorkRunner implements Runnable {
		...
		/** 工作过程 */
		private AsyncIoWorkProcNotID _workProc = null;
		 ...
		 * 
		 */
		public IoWorkRunner(AsyncIoWorkProcNotID workProc) {
			if (workProc == null) {
		...
		public void run() {
			while (true) {
				try {
					...
					// 记录异常信息
					IoLogger.theInstance().logError(new XgameIoError("AsyncIoWorkProcNotID::IoWorkRunner error", ex));
				}
			}
		}
	}
