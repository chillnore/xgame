...
/**
 * An {@link MsgBufferAllocator} that caches the buffers which are likely to
 * be reused during auto-expansion of the buffers.
 * <p>
 * In {@link SimpleBufferAllocator}, the underlying {@link ByteBuffer} of
 * the {@link MsgBuffer} is reallocated on its capacity change, which means
 ...
 * performance advantage if {@link MsgBuffer#free()} is called properly.
 *
 * @author <a href="http://mina.apache.org">Apache MINA Project</a>
 */
public class CachedBufferAllocator implements MsgBufferAllocator {
...
//		int poolSize = maxPoolSize == 0? DEFAULT_MAX_POOL_SIZE : maxPoolSize;
    ...
    public MsgBuffer allocate(int requestedCapacity, boolean direct) {
        int actualCapacity = MsgBuffer.normalizeCapacity(requestedCapacity);
        MsgBuffer buf ;
    ...
    public MsgBuffer wrap(ByteBuffer nioBuffer) {
    ...
    private class CachedBuffer extends AbstractMsgBuffer {
        ...
        @Override
        protected MsgBuffer duplicate0() {
            return new CachedBuffer(this, buf().duplicate());
        }

        @Override
        protected MsgBuffer slice0() {
            return new CachedBuffer(this, buf().slice());
        }

        @Override
        protected MsgBuffer asReadOnlyBuffer0() {
